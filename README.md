...
| `--env <env>` | `-e` | 指定环境：dev, test, pro | 全部环境 |
...
# 输出: .env-dev, .env-test, .env-pro

# 仅生成 dev 环境
envdog generate --env dev

# 生成并替换配置文件（会创建 .bak 备份）
envdog generate --replace

# 仅生成 dev 环境并替换
envdog generate --env dev --replace
...
从 `.env` 文件还原明文值到配置文件。用于提交代码前临时恢复明文值以便本地运行测试。
...
将配置文件中的明文值重新替换为环境变量占位符。用于提交完成后重新保护敏感值。
...
```json
{
  ...
  "varMappings": {
    "spring.datasource.url": {
      ...
      "dev": "DATASOURCE_URL_DEV"
    },
    "spring.datasource.username": {
      "default": "DATASOURCE_USERNAME",
      "dev": "DATASOURCE_USERNAME_DEV"
    },
    "spring.datasource.password": {
      "default": "DATASOURCE_PASSWORD",
      "dev": "DATASOURCE_PASSWORD_DEV"
    }
  }
}
```
...
`.env-dev`:
```bash
DATASOURCE_URL_DEV=jdbc:mysql://dev-server:3306/mydb
DATASOURCE_USERNAME_DEV=dev_admin
DATASOURCE_PASSWORD_DEV=dev_secret
```

`.env-pro`:
```bash
DATASOURCE_URL_PRO=jdbc:mysql://pro-server:3306/mydb
DATASOURCE_USERNAME_PRO=pro_admin
DATASOURCE_PASSWORD_PRO=pro_secret
```
...
# 开发环境（DEV_ 前缀）
DEV_DATASOURCE_URL_DEV=jdbc:mysql://dev-server:3306/mydb
DEV_DATASOURCE_USERNAME_DEV=dev_admin
DEV_DATASOURCE_PASSWORD_DEV=dev_secret

# 测试环境（TEST_ 前缀）
TEST_DATASOURCE_URL_TEST=jdbc:mysql://test-server:3306/mydb
TEST_DATASOURCE_USERNAME_TEST=test_admin
TEST_DATASOURCE_PASSWORD_TEST=test_secret

# 生产环境（PRO_ 前缀）
PRO_DATASOURCE_URL_PRO=jdbc:mysql://pro-server:3306/mydb
PRO_DATASOURCE_USERNAME_PRO=pro_admin
PRO_DATASOURCE_PASSWORD_PRO=pro_secret
```
